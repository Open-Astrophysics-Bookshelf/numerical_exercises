...
derivatives for a standard Laplacian discretization.}
\end{figure}
%
...
on cell edges:
\begin{align}
\left . \frac{d\phi}{dx} \right |_{i-\myhalf} &= \frac{\phi_i - \phi_{i-1}}{\Delta x}\\
\left . \frac{d\phi}{dx} \right |_{i+\myhalf} &= \frac{\phi_{i+1} - \phi_i}{\Delta x}
\end{align}
...
\begin{equation}
\left . \frac{d^2\phi}{dx^2} \right |_i = \frac{ d\phi/dx |_{i+\myhalf} - d\phi/dx |_{i-\myhalf}}{\Delta x}
\end{equation}
...
$\phi$ on edges:
\begin{equation}
[\nabla \phi \cdot \hat{x}]_{i+\myhalf,j} = \frac{\phi_{i+1,j} - \phi_{i,j}}{\Delta x}
\end{equation}
...
[\nabla^2 \phi]_{i,j} &=
   \frac{[\nabla \phi \cdot \hat{x}]_{i+\myhalf,j} -
         [\nabla \phi \cdot \hat{x}]_{i-\myhalf,j}}{\Delta x} +
   \frac{[\nabla \phi \cdot \hat{y}]_{i,j+\myhalf} -
         [\nabla \phi \cdot \hat{y}]_{i,j-\myhalf}}{\Delta y} \nonumber\\
%
...
\caption[Smoothing of different wavenumbers]{\label{fig:smooth} Error
  ...
  \hydroexdoit{\href{https://github.com/zingale/hydro_examples/blob/master/multigrid/smooth-modes.py}{smooth-modes.py}}}
\end{figure}
...
\caption[The geometry for 1-d
  prolongation and restriction]{\label{fig:1dgrid-prolong} A fine grid and
