...
def simplegrid():
    ...
    pylab.text(xr[nzones/2]-0.075*dx, yc[nzones/2], r"$a^{n+1/2}_{i+1/2,j,L}$", 
               fontsize="15", rotation="270",
               horizontalalignment='right', verticalalignment='center')

    pylab.scatter(xl[nzones/2+1]+0.05*dx, yc[nzones/2], marker="x", s=50)
    pylab.text(xl[nzones/2+1]+0.075*dx, yc[nzones/2], r"$a^{n+1/2}_{i+1/2,j,R}$", 
               fontsize="15", rotation="270",
    ...
    pylab.text(xc[nzones/2], yr[nzones/2]-0.075*dy, r"$a^{n+1/2}_{i,j+1/2,L}$", 
               fontsize="15", rotation="0",
               horizontalalignment='center', verticalalignment='top')

    pylab.scatter(xc[nzones/2], yl[nzones/2+1]+0.05*dx, marker="x", s=50)
    pylab.text(xc[nzones/2], yl[nzones/2+1]+0.075*dy, r"$a^{n+1/2}_{i,j+1/2,R}$", 
               fontsize="15", rotation="0",
    ...
    pylab.savefig("2dgrid.png")
    pylab.savefig("2dgrid.eps")
