...
\caption{\label{fig:adv:gaussian-2d} Advection of a Gaussian profile
  with $u = v = 1$ for one period on a $32\times 32$ grid, with $C =
...
\caption{\label{fig:adv:tophat-2d} Advection of a tophat function with
  $u = v = 1$ for one period on a $32\times 32$ grid, with $C = 0.8$.
...
C \le \frac{1}{d}
\end{equation}
We demonstrate this in Figure~\ref{fig:adv:tophat-2d-rk4}, using
piecewise linear reconstruction and the method-of-lines integration in
time with 4th order Runge-Kutta.  We see that the case with $C = 0.8$
is unstable, but the $C = 0.4$ case is stable.

See \cite{titarevtoro} for some discussion.  Alternate integrators and
stencils may be able to alleviate this.

\begin{figure}
\centering
\includegraphics[width=0.48\linewidth]{tophat_rk4_cfl08}\hspace{1em}
\includegraphics[width=0.48\linewidth]{tophat_rk4_cfl04}
\caption{\label{fig:adv:tophat-2d-rk4} Advection of a tophat function using
  ...
  is $C = 0.8$---this is clearly unstable.  On the right is $C = 0.4$,
  which is stable.  
