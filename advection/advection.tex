...
\begin{figure}[t]
\centering
% figure generated by codes/advection/fd/fdadvect.py
...
{Convergence for the second-order finite-volume method with minmod
limiting advecting a Gaussian initial profile with $C = 0.8$.}
\end{figure}
...
\begin{eqnarray}
F^{(x)}_{i+1/2,j} &=& \frac{1}{\Delta y \Delta t}
    \int_{t^n}^{t^{n+1}} \int_{y_{j-1/2}}^{y_{j+1/2}} (ua)_{i+1/2,j} dy dt  \\
F^{(y)}_{i,j+1/2} &=& \frac{1}{\Delta x \Delta t}
