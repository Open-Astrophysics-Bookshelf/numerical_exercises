...
This looks like a mess. However, the whole point of having a function \emph{basis}
is to use linear algebra techniques on the basis functions $\psi_n$. By using
...
\begin{equation}
  ...
  \sum_n \left[ \left( \int_V \psi_m(x) \psi_n(x) \right) \left( \ddt{\hat{a}_n} - \hat{\epsilon}_n \right) + \left( \int_{\partial V} \hat{a}_n \psi_m(x) \psi_n(x) \right) - \left( \int_V \hat{a}_n \psi_n(x) \ddx{\psi_m}(x) \right) \right] = 0.
\end{equation}
%
