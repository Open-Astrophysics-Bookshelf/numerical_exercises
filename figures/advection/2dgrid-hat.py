...
def simplegrid():
    ...
    # i+1/2,j interface
    gr.mark_cell_left_state_x(nzones/2, nzones/2, r"$\hat{a}^{n+1/2}_{i+1/2,j,L}$", color="b")

    # i,j+1/2 interface
    gr.mark_cell_left_state_y(nzones/2, nzones/2, r"$\hat{a}^{n+1/2}_{i,j+1/2,L}$", color="b")
    gr.mark_cell_right_state_y(nzones/2, nzones/2+1, r"$\hat{a}^{n+1/2}_{i,j+1/2,R}$", color="b")

    # i,j-1/2 interface
    gr.mark_cell_left_state_y(nzones/2, nzones/2-1, r"$\hat{a}^{n+1/2}_{i,j-1/2,L}$", color="b")
    gr.mark_cell_right_state_y(nzones/2, nzones/2, r"$\hat{a}^{n+1/2}_{i,j-1/2,R}$", color="b")
