...
\usepackage{fancyhdr}
\pagestyle{fancy}
\fancyfoot[LO,LE]{\footnotesize \color{mygray} M.\ Zingale---Notes on incompressible solvers}
\fancyfoot[RO,RE]{\footnotesize \color{mygray} (\today)}
...
fluid objects the velocity constraint:
\begin{equation}
\nabla \cdot U = 0
\end{equation}
...
Minion introduced a simple test problem with an analytic solution.
