...
\begin{equation}
\Uc^{n+1}_i = \Uc^n_i + \frac{\Delta t}{\Delta x} \left ( \Fb_{i-1/2}^{n+1/2} - \Fb_{i+1/2}^{n+1/2} \right )
\end{equation}
...
approximation to the state on the interface, $\Uc_{i-1/2}^{n+1/2}$ and
$\Uc_{i+1/2}^{n+1/2}$ and use this with the flux function to define the
flux through the interface:
\begin{align}
\Fb_{i-1/2}^{n+1/2} &= \Fb(\Uc_{i-1/2}^{n+1/2}) \\
\Fb_{i+1/2}^{n+1/2} &= \Fb(\Uc_{i+1/2}^{n+1/2})
\end{align}
...
\begin{equation}
\Uc_{i+1/2,L}^{n+1/2} = \Uc(\qb_{i+1/2,L}^{n+1/2})
\end{equation}
...
\begin{equation}
\qb(x) = \alpha (x - x_i)^2 + \beta (x - x_i) + \gamma
\end{equation}
...
\begin{equation}
\qb_{i+1/2,L}^{n+1/2} = \qb_i^n +
     \frac{1}{2} \left [ RL - \frac{\Delta t}{\Delta x} R\Lambda L \right ]_i \overline{\Delta \qb}_i
\end{equation}
We see the common factor of $L \Delta \qb$.  We now write this back in
...
\end{equation}
Starting with $L \overline{\Delta \qb}$, which is a vector with each component
...
R\Lambda L \overline{\Delta \qb} =
   ...
   \left ( \begin{array}{ccc}
             \lambda^\evm &              & \\
                          ...
                          &              & \lambda^\evp \end{array} \right )
   \left ( \begin{array}{c}
            l^\evm \cdot \overline{\Delta \qb} \\
            l^\evz \cdot \overline{\Delta \qb} \\
            l^\evp \cdot \overline{\Delta \qb} \end{array} \right )
\renewcommand{\arraystretch}{1.0}
\end{equation}
...
R\Lambda L \overline{\Delta \qb} =
   \left ( \begin{array}{c:c:c}
             ...
             r_3^\evm & r_3^\evz & r_3^\evp \end{array} \right )
   \left ( \begin{array}{c}
            \lambda^\evm \, l^\evm \cdot \overline{\Delta \qb} \\
            \lambda^\evz \, l^\evz \cdot \overline{\Delta \qb} \\
            \lambda^\evp \, l^\evp \cdot \overline{\Delta \qb} \end{array} \right )
\renewcommand{\arraystretch}{1.0}
\end{equation}
...
R\Lambda L \overline{\Delta \qb} =
   \left ( \begin{array}{c}
            r_1^\evm \, \lambda^\evm \, l^\evm \cdot \overline{\Delta \qb} +
            r_1^\evz \, \lambda^\evz \, l^\evz \cdot \overline{\Delta \qb} +
            r_1^\evp \, \lambda^\evp \, l^\evp \cdot \overline{\Delta \qb} \\
%
            r_2^\evm \, \lambda^\evm \, l^\evm \cdot \overline{\Delta \qb} +
            r_2^\evz \, \lambda^\evz \, l^\evz \cdot \overline{\Delta \qb} +
            r_2^\evp \, \lambda^\evp \, l^\evp \cdot \overline{\Delta \qb} \\
%
            r_3^\evm \, \lambda^\evm \, l^\evm \cdot \overline{\Delta \qb} +
            r_3^\evz \, \lambda^\evz \, l^\evz \cdot \overline{\Delta \qb} +
            r_3^\evp \, \lambda^\evp \, l^\evp \cdot \overline{\Delta \qb} \\
   \end{array} \right )
\renewcommand{\arraystretch}{1.0}
\end{equation}
We can rewrite this compactly as:
\begin{equation}
\sum_\nu \lambda^{(\nu)} (l^{(\nu)} \cdot \overline{\Delta \qb}) r^{(\nu)}
\end{equation}
where we use $\nu$ to indicate which wave we are summing over.  A similar
expansion is used for $RL \overline{\Delta \qb}$.  In fact, any vector
...
can be expressed as:
\begin{equation}
\Ab \Delta \qb =  \Ab \sum_\nu (l^\enu \cdot \overline{\Delta \qb}) r^\enu = \sum_\nu (l^\enu \cdot \overline{\Delta \qb}) \Ab r^\enu = \sum_\nu (l^\enu \cdot \overline{\Delta \qb}) \lambda^\enu r^\enu
\end{equation}
where we used $\Ab r^\enu = \lambda^\enu r^\enu$.  The quantity $(l^\enu
...
\begin{equation}
\qb_{i+1/2,L}^{n+1/2} = \qb_i^n + \frac{1}{2} \sum_{\nu; \lambda^{(\nu)} \ge 0}
  \left [ 1 - \frac{\Delta t}{\Delta x} \lambda_i^{(\nu)} \right ] (l_i^{(\nu)} \cdot \overline{\Delta \qb}_i) r_i^{(\nu)}
\end{equation}
...
\begin{equation}
\qb_{i+1/2,R}^{n+1/2} = \qb_{i+1}^n - \frac{1}{2} \sum_{\nu; \lambda^{(\nu)} \le 0}
  \left [ 1 + \frac{\Delta t}{\Delta x} \lambda_{i+1}^{(\nu)} \right ] (l_{i+1}^{(\nu)} \cdot \overline{\Delta \qb}_{i+1}) r_{i+1}^{(\nu)}
\end{equation}
...
\begin{equation}
\qb_{i+1/2,L}^{n+1/2} - \qb_\mathrm{ref} = \qb_i^n - \qb_\mathrm{ref} +
...
us how much of the quantity $\qb_{i+1/2,L}^{n+1/2} - \qb_\mathrm{ref}$
...
\begin{equation}
\qb_{i+1/2,L}^{n+1/2} - \tilde{\qb}_{i,L} = \frac{1}{2} \frac{\Delta t}{\Delta x}
...
\begin{equation}
\qb_{i+1/2,R}^{n+1/2} = \tilde{\qb}_{i+1,R} + \frac{1}{2} \frac{\Delta t}{\Delta x}
       \sum_{\nu; \lambda^{(\nu)} \le 0}
...
then compute $\qb_{i-1/2,R}^{n+1/2}$ together with $\qb_{i+1/2,L}^{n+1/2}$
...
\begin{equation}
\frac{1}{\Delta x} \int_{x_{i-1/2}}^{x_{i+1/2}} \qb(x) dx = \qb_i
\end{equation}
...
of the zone ($\qb_{i-1/2,R}^{n+1/2}$).  Here, the integral under the parabolic
...
\begin{equation}
\qb_{i-1/2,R}^{n+1/2} = \tilde{\qb}_- - \sum_{\nu; \lambda_\nu \le 0}
...
\subsection{Flattening and contact steepening}
  ...
  \begin{equation}
  \qb_{i+1/2,\{L,R\}}^{n+1/2} \leftarrow (1 - \chi) \qb_i + \chi \qb_{i+1/2,\{L,R\}}^{n+1/2}
  \end{equation}
...
\subsection{Limiting on characteristic variables}
  ...
  \qb \end{equation} any limiting would then be done to $\Delta
  w^{(\nu)}$ and the limited primitive variables would be recovered
  as: \begin{equation} \overline{\Delta \qb} = \sum_\nu \overline{\Delta
...
\begin{equation}
\qb_{i+1/2}^{n+1/2} = \mathcal{R}(\qb_{i+1/2,L}^{n+1/2}, \qb_{i+1/2,R}^{n+1/2})
\end{equation}
...
\begin{equation}
\renewcommand{\arraystretch}{1.5}
\Fb_{i+1/2}^{n+1/2} = \left ( \begin{array}{c}
...
\Uc^{n+1}_i = \Uc^n_i + \frac{\Delta t}{\Delta x} 
   \left ( \Fb_{i-1/2}^{n+1/2} - \Fb_{i+1/2}^{n+1/2} \right )
\end{equation}
...
\begin{align}
\Uc_{i+1/2,j,L}^{n+1/2} &= \Uc_{i,j}^n + \frac{\Delta x}{2} \frac{\partial \Uc}{\partial x}
...
the normal state:
\begin{align}
\hat{\Uc}_{i+1/2,j,L}^{n+1/2} &\equiv \Uc_{i,j}^n
      + \frac{1}{2} \left [ 1 - \frac{\Delta t}{\Delta x} \Ab^{(x)}(\Uc) \right ] \Delta \Uc \\
\Uc_{i+1/2,j,L}^{n+1/2} &= \hat{\Uc}_{i+1/2,j,L}^{n+1/2}
...
We note here that $\hat{\Uc}_{i+1/2,j,L}^{n+1/2}$ is essentially
...
and compute this as:
\begin{equation}
\hat{\Uc}_{i+1/2,j,L}^{n+1/2} = \Uc(\hat{\qb}_{i+1/2,j,L}^{n+1/2})
\end{equation}
...
\begin{equation}
\hat{\Uc}_{i,j+1/2,L}^{n+1/2} = \Uc(\hat{\qb}_{i,j+1/2,L}^{n+1/2})
\end{equation}
...
for computing the full $x$-interface left state, $\Uc_{i+1/2,j,L}^{n+1/2}$, we need the
transverse ($y$) states, which we define as
\begin{align}
\Uc^T_{i,j+1/2} &= \mathcal{R}(\hat{\Uc}^{n+1/2}_{i,j+1/2,L},
                            \hat{\Uc}^{n+1/2}_{i,j+1/2,R}) \\
\Uc^T_{i,j-1/2} &= \mathcal{R}(\hat{\Uc}^{n+1/2}_{i,j-1/2,L},
                            \hat{\Uc}^{n+1/2}_{i,j-1/2,R})
\end{align}
...
\begin{equation}
\Uc_{i+1/2,j,L}^{n+1/2} = \Uc(\hat{\qb}_{i+1/2,j,L}^{n+1/2})
   - \frac{\Delta t}{2} \frac{\Fb^{(y)}(\Uc^T_{i,j+1/2}) - \Fb^{(y)}(\Uc^T_{i,j-1/2})}{\Delta y}
\end{equation}
...
\begin{equation}
\Uc_{i+1/2,j,R}^{n+1/2} = \Uc(\hat{\qb}_{i+1/2,j,R}^{n+1/2})
   - \frac{\Delta t}{2} \frac{\Fb^{(y)}(\Uc^T_{i+1,j+1/2}) - \Fb^{(y)}(\Uc^T_{i+1,j-1/2})}{\Delta y}
\end{equation}
...
\begin{equation}
\Uc_{i+1/2,j}^{n+1/2} = \mathcal{R}(\Uc_{i+1/2,j,L}^{n+1/2}, \Uc_{i+1/2,j,R}^{n+1/2})
\end{equation}
...
\begin{equation}
\Fb^{(x),n+1/2}_{i+1/2,j} = \Fb^{(x)}(\Uc_{i+1/2,j}^{n+1/2})
\end{equation}
...
\Uc^{n+1}_{i,j} = \Uc^n_{i,j}
   + \frac{\Delta t}{\Delta x} \left ( \Fb^{(x),n+1/2}_{i-1/2,j} - \Fb^{(x),n+1/2}_{i+1/2,j} \right )
   + \frac{\Delta t}{\Delta y} \left ( \Fb^{(y),n+1/2}_{i,j-1/2} - \Fb^{(y),n+1/2}_{i,j+1/2} \right )
\end{equation}
...
$u$, not $\rho$ or $p$\footnote{Note that in the PPM paper, they put $G$ on
...
now:
\begin{equation}
\sum_{\nu; \lambda^\enu \ge 0}l^\enu \cdot (\tilde{\qb} - \Ic^\enu_+(\qb) - \tfrac{\Delta t}{2} G) r^\enu
\end{equation}
for the left state, and
\begin{equation}
\sum_{\nu; \lambda^\enu \le 0} l^\enu \cdot (\tilde{\qb} - \Ic^\enu_-(\qb) - \tfrac{\Delta t}{2} G) r^\enu
\end{equation}
...
\Uc^{n+1}_{i,j} = \Uc^n_{i,j}
   &+ \frac{\Delta t}{\Delta x} \left ( \Fb^{(x),n+1/2}_{i-1/2,j} - \Fb^{(x),n+1/2}_{i+1/2,j} \right ) \nonumber \\
   &+ \frac{\Delta t}{\Delta y} \left ( \Fb^{(y),n+1/2}_{i,j-1/2} - \Fb^{(y),n+1/2}_{i,j+1/2} \right ) + \frac{\Delta t}{2} \left (H(\Uc^n_{i,j}) + H(\Uc^{n+1}_{i,j})\right )
\end{align}
As written, this appears to be an implicit update (since $\Uc^{n+1}$
depends on $H^{n+1}$), but often, the form of the source terms allows
...
and $H = (0, \rho g, \rho u g)^\intercal$, so our update sequence is:
\begin{align}
\rho^{n+1}_i = \rho^n_i &+ \frac{\Delta t}{\Delta x} 
   \left [\rho_{i-1/2}^{n+1/2} u_{i-1/2}^{n+1/2} - 
          \rho_{i+1/2}^{n+1/2} u_{i+1/2}^{n+1/2} \right ] \\
(\rho u)^{n+1}_i = (\rho u)^n_i &+ \frac{\Delta t}{\Delta x} 
   \left [ \rho_{i-1/2}^{n+1/2} (u_{i-1/2}^{n+1/2})^2  - 
           \rho_{i+1/2}^{n+1/2} (u_{i+1/2}^{n+1/2})^2  \right ]
         + \frac{\Delta t}{\Delta x} \left ( p_{i-1/2}^{n+1/2} - 
                                              p_{i+1/2}^{n+1/2} \right ) \nonumber \\
       &+ \frac{\Delta t}{2}(\rho^n_i + \rho^{n+1}_i) g \\
(\rho E)^{n+1}_i = (\rho E)^n_i &+ \frac{\Delta t}{\Delta x} 
   \left [ \left (\rho_{i-1/2}^{n+1/2} E_{i-1/2}^{n+1/2}  + p_{i-1/2}^{n+1/2} \right ) u_{i-1/2}^{n+1/2} - \right . \nonumber \\
   &\phantom{+ \frac{\Delta t}{\Delta x} \left[ \right.}   \left .   \left (\rho_{i+1/2}^{n+1/2} E_{i+1/2}^{n+1/2}  + p_{i+1/2}^{n+1/2} \right ) u_{i+1/2}^{n+1/2} \right ]
...
\begin{equation}
\frac{d\Uc_{i,j}}{dt} = -\frac{\Fb^{(x)}(\Uc_{i+1/2,j}) - \Fb^{(x)}(\Uc_{i-1/2,j})}{\Delta x}
                      -\frac{\Fb^{(y)}(\Uc_{i,j+1/2}) - \Fb^{(y)}(\Uc_{i,j-1/2})}{\Delta y}
\end{equation}
...
\begin{alignat}{3}
k_1 &= \Delta t \bigg [ &-&\frac{\Fb^{(x)}(\Uc^n_{i+1/2,j})
                               - \Fb^{(x)}(\Uc^n_{i-1/2,j})}{\Delta x} \nonumber \\
    &\,                 &-&\frac{\Fb^{(y)}(\Uc^n_{i,j+1/2})
                               - \Fb^{(y)}(\Uc^n_{i,j-1/2})}{\Delta y} \bigg ] \\
k_2 &= \Delta t \bigg [ &-&\frac{\Fb^{(x)}([\Uc^n + k_1/2]_{i+1/2,j})
                               - \Fb^{(x)}([\Uc^n + k_1/2]_{i-1/2,j})}{\Delta x} \nonumber \\
    &\,                 &-&\frac{\Fb^{(y)}([\Uc^n + k_1/2]_{i,j+1/2})
                               - \Fb^{(y)}([\Uc^n + k_1/2]_{i,j-1/2})}{\Delta y} \bigg ]
\end{alignat}
and then
\begin{equation}
\Uc^{n+1}_{i,j} = \Uc^n_{i,j} + k_2
\end{equation}
In the construction of the interface states, $\Uc^n_{i+1/2,j}$ or $[\Uc^n
...
\begin{equation}
r^\evm = \left ( \begin{array}{c} 1 \\ -c/\rho \\ c^2 \\ 0\end{array} \right )
%
\qquad
r^\evz = \left ( \begin{array}{c} 1 \\ 0 \\ 0  \\ 0\end{array} \right )
%
\qquad
r^\evzs{X} = \left ( \begin{array}{c} 0 \\ 0  \\ 0 \\ 1 \end{array} \right )
%
\qquad
r^\evp = \left ( \begin{array}{c} 1 \\ c/\rho \\ c^2 \\ 0 \end{array} \right )
\end{equation}
...
eigenvectors ($r^\evzs{X}$).  This means that $X_k$ only jumps over
...
\begin{equation}
r^\evm = \left( \begin{array}{c} 1\\ {c}/{\tau}\\ -{c^{2}}/{\tau^{2}}\\ {\alpha}/{\tau}\end{array}\right)
%
\qquad
r^\evz = \left( \begin{array}{c} 1\\0\\0\\0 \end{array}\right)
%
\qquad
r^\evzs{\gamma_e} = \left( \begin{array}{c} 0\\0\\0\\1\end{array}\right)
%
\qquad
r^\evp = \left( \begin{array}{c} 1\\ -{c}/{\tau}\\ -{c^{2}}/{\tau^{2}}\\ {\alpha}/{\tau} \end{array}\right )
\end{equation}
and corresponding left eigenvectors are:
\begin{align}
l^\evm &= \left( \begin{array}{cccc} 0 & \frac{\tau}{2 c} & - \frac{\tau^{2}}{2 c^{2}} & 0\end{array}\right) \\
%
l^\evz &= \left( \begin{array}{cccc} 0 & \frac{\tau}{2 c} & - \frac{\tau^{2}}{2 c^{2}} & 0\end{array}\right) \\
%
l^\evzs{\gamma_e} &= \left( \begin{array}{cccc} 0 & 0 & \frac{\alpha \tau}{c^{2}} & 1\end{array}\right) \\
%
l^\evp &= \left ( \begin{array}{cccc} 0 & - \frac{\tau}{2 c} & - \frac{\tau^{2}}{2 c^{2}} & 0\end{array}\right)
\end{align}
...
other states by $l \cdot \qb$.  Since all $l^{(\nu)}$'s have $0$ in the
