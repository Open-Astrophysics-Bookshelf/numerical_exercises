...
\begin{equation}
\Uc_{i+1/2,L}^{n+1/2} = \Uc(q_{i+1/2,L}^{n+1/2})
\end{equation}
...
variables, $\Uc$, to the primitive variables, $q$.  These have a simpler
...
&= \Uc_{i,j}^n + \frac{1}{2} \left [ 1 - \frac{\Delta t}{\Delta x} A^{(x)}(\Uc) \right ] \Delta \Uc
                            - \frac{\Delta t}{2} \frac{\partial \Fb^{(y)}}{\partial y} \label{eq:Utaylorstate}
\end{align}
where $A^{(x)}(\Uc) \equiv \partial \Fb^{(x)} / \partial \Uc$.  We decompose
...
\hat{\Uc}_{i+1/2,j,L}^{n+1/2} &\equiv \Uc_{i,j}^n
      + \frac{1}{2} \left [ 1 - \frac{\Delta t}{\Delta x} A^{(x)}(\Uc) \right ] \Delta \Uc \\
...
$A^{(x)}(\Uc)$).  This means that we can compute this term using the
...
and compute this as:
\begin{equation}
\hat{\Uc}_{i+1/2,j,L}^{n+1/2} = \Uc(\hat{q}_{i+1/2,j,L}^{n+1/2})
\end{equation}
...
\begin{equation}
\hat{\Uc}_{i,j+1/2,L}^{n+1/2} = \Uc(\hat{q}_{i,j+1/2,L}^{n+1/2})
\end{equation}
...
\begin{equation}
\Uc_{i+1/2,j,L}^{n+1/2} = \Uc(\hat{q}_{i+1/2,j,L}^{n+1/2})
...
\begin{equation}
\Uc_{i+1/2,j,R}^{n+1/2} = \Uc(\hat{q}_{i+1/2,j,R}^{n+1/2})
...
our system as
\begin{equation}
q = \left ( \begin{array}{c} \tau \\ u \\ p \\ \gamma_e \end{array} \right )
\end{equation}
(we use $\tau = 1/\rho$ here for consistency with CG), we have
\begin{equation}
A = \left ( \begin{array}{cccc} u & -\tau    & 0      & 0 \\
                                0 & u        & \tau   & 0 \\
                                0 & c^2/\tau & u      & 0 \\
                                0 & -\alpha  & 0      & u
            \end{array} \right )
