...
now:
\begin{equation}
\sum_{\nu; \lambda^\enu \ge 0}\lb^\enu \cdot (\tilde{\qb} - \Ic^\enu_+(\qb) - \tfrac{\Delta t}{2} G) \rb^\enu
\end{equation}
for the left state, and
\begin{equation}
\sum_{\nu; \lambda^\enu \le 0} \lb^\enu \cdot (\tilde{\qb} - \Ic^\enu_-(\qb) - \tfrac{\Delta t}{2} G) \rb^\enu
\end{equation}
...
\begin{alignat}{3}
{\bf k}_1 &= \Delta t \bigg [ &-&\frac{\Fb^{(x)}(\Uc^n_{i+1/2,j})
                               - \Fb^{(x)}(\Uc^n_{i-1/2,j})}{\Delta x} \nonumber \\
    &\,                 &-&\frac{\Fb^{(y)}(\Uc^n_{i,j+1/2})
                               - \Fb^{(y)}(\Uc^n_{i,j-1/2})}{\Delta y} \bigg ] \\
{\bf k}_2 &= \Delta t \bigg [ &-&\frac{\Fb^{(x)}([\Uc^n + {\bf k}_1/2]_{i+1/2,j})
                               - \Fb^{(x)}([\Uc^n + {\bf k}_1/2]_{i-1/2,j})}{\Delta x} \nonumber \\
    &\,                 &-&\frac{\Fb^{(y)}([\Uc^n + {\bf k}_1/2]_{i,j+1/2})
                               - \Fb^{(y)}([\Uc^n + {\bf k}_1/2]_{i,j-1/2})}{\Delta y} \bigg ]
\end{alignat}
and then
\begin{equation}
...
+ {\bf k}_1/2]_{i+1/2,j}$, there is no explicit transverse term, since that
...
includes the transverse information since we add ${\bf k}_1/2$ to
