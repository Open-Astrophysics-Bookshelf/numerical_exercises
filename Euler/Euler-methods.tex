...
U^{n+1}_{i,j} = U^n_{i,j}
   &+ \frac{\Delta t}{\Delta x} \left ( F^{(x),n+1/2}_{i-1/2,j} - F^{(x),n+1/2}_{i+1/2,j} \right ) \nonumber \\
   &+ \frac{\Delta t}{\Delta y} \left ( F^{(y),n+1/2}_{i,j-1/2} - F^{(y),n+1/2}_{i,j+1/2} \right ) + \frac{\Delta t}{2} \left (H(U^n_{i,j}) + H(U^{n+1}_{i,j})\right )
\end{align}
As written, this appears to be an implicit update (since $U^{n+1}$
...
looking in 1-d for simplicity, $U = (\rho, \rho u, \rho E)^\intercal$
