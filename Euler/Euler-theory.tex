...
flux \MarginPar{what about for a general EOS?}  vector in terms of the
conserved variables directly.  Taking $m \equiv \rho u$, $\mathcal{E}
\equiv \rho E$, and assuming a gamma-law EOS%
...
\caption[The Sod problem]{\label{fig:sod} Evolution following from an initial
  ...
  waves carrying changes in the solution.{ \color{red}  redo}}
\end{figure}
...
These are completely general at this point.  Finding the solution
across the rarefaction then involves integrating the system from
$p_{l,r}$ to $p_\star$, where $l, r$ is the original left or right
state, depending on which wave you are considering.  Coupled with
this, we need our equation of state to provide $C = C(\tau, p)$.
These solutions are called the {\em Riemann invariants}.
...
p = K \rho^\gamma
\end{equation}
where $K$ is a constant that depends on the entropy.
