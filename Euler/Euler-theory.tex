...
\end{equation}
Finally, defining $dw = \Lb d\qb$,
\begin{equation}
w_t + \Lambda w_x = 0
\end{equation}
Here, the $w$ are the {\em characteristic variables}.  Note that we cannot
in general integrate $dw = \Lb d\qb$ to write down the characteristic
...
could then get back the primitive variables as $d\qb = \Lb^{-1} dw = \Rb dw$.
...
variables are $\Delta w \sim \Lb \Delta \qb$ (where the $\sim$ accounts
...
carry its respective jump, $\Delta w$.  Since $d\qb = \Lb^{-1}dw = \Rb dw$,
