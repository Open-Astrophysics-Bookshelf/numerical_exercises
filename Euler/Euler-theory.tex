...
= u_\star^\mathrm{shock}(p_\star)$.  The general solution procedure is starts with
...
With this, show that the shock speed is:
\begin{equation}
...
x_\mathrm{interface})/t$.  The initial discontinuity it on the
interface, corresponding to $\xi = 0$.
...
\caption{\label{fig:euler:riemann_sample} An illustration of the 3
  waves eminating from an initial discontinuity (the origin of the
  axes).  The 4 cases show all possible cases of waves being to the
  left or right of the interface.  In all cases, the middle wave is a
  contact and the left (1) and right (3) waves are either a shock or a
  rarefaction.  The state on the interface is: $R$ (top-left);
  $R\star$ (top-right); $L\star$ (bottom-left); $L$ (bottom-right).}
\end{figure}
