...
The eigenvalues of $\Ab$ can be found via $| \Ab - \lambda I | = 0$,
...
\begin{equation}
\Ab \, r^\enu = \lambda^\enu r^\enu \; ;
\qquad
l^\enu \, \Ab  = \lambda^\enu l^\enu
\end{equation}
...
we wish to diagonalize the matrix $\Ab$.  We take the matrix $R$ to be the
...
{
Show that $\Lambda = L \Ab R$ is a diagonal matrix with the diagonal elements
...
\begin{equation}
L\qb_t + L \Ab R L \qb_x = 0
\end{equation}
Finally, defining $dw = L d\qb$,
\begin{equation}
w_t + \Lambda w_x = 0
\end{equation}
Here, the $w$ are the {\em characteristic variables}.  Note that we cannot
in general integrate $dw = L d\qb$ to write down the characteristic
...
could then get back the primitive variables as $d\qb = L^{-1} dw = R dw$.
...
variables are $\Delta w \sim L \Delta \qb$ (where the $\sim$ accounts
for the fact that in a nonlinear system, $L = L(\qb)$).  The
characteristic form of the equations says that each of the waves will
carry its respective jump, $\Delta w$.  Since $d\qb = L^{-1}dw = R dw$,
