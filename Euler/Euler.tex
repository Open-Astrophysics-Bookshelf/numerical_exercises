...
\begin{quote}
\noindent {\em These notes describe how to do a high-order solution to
  the Euler equations by compiling the discussions of several
  papers/books.}
\end{quote}
...
\end{eqnarray}
These represent conservation of mass, momentum, and energy.  Here $E$
...
\begin{quote}
{\em Exercise 2: 
Show that the eigenvalues of $A$ are $\lambda_1 = u -c$, $\lambda_2 = u$, $\lambda_3 = u+c$ where the speed of sound is $c = \sqrt{\gamma p /\rho}$.
}
\end{quote}
These eigenvalues are the speeds at which information propagates for the
fluids equations.
Since the eigenvalues are real, this system (the Euler equations) are said
...
l^{(k)} \, A  = \lambda_k l^{(k)}
\end{equation}
where there is one right and one left eigenvector for each of the eigenvalues.
%
\begin{quote}
{\em Exercise 3: 
Show that the right eigenvectors are:
\begin{equation}
r^{(1)} = \left ( \begin{array}{c} 1 \\ -c/\rho \\ c^2 \end{array} \right )
%
\qquad
r^{(2)} = \left ( \begin{array}{c} 1 \\ 0 \\ 0  \end{array} \right )
%
\qquad
r^{(3)} = \left ( \begin{array}{c} 1 \\ c/\rho \\ c^2 \end{array} \right )
\end{equation}
and the left eigenvectors are:
\begin{equation}
l^{(1)} = \left ( \begin{array}{ccc} 0 & -\frac{\rho}{2c} & \frac{1}{2c^2} 
                  \end{array} \right )
%
\qquad
l^{(2)} = \left ( \begin{array}{ccc} 1 & 0 & -\frac{1}{c^2}  \end{array} \right )
%
\qquad
l^{(3)} = \left ( \begin{array}{ccc} 0 & \frac{\rho}{2c} & \frac{1}{2c^2} \end{array} \right )
\end{equation}
...
matrix of right eigenvectors, $R = (r^{(1)} | r^{(2)} | r^{(3)} )$, and $L$
is the corresponding matrix of left eigenvectors.  Note that $L\, R = I$, and
$L = R^{-1}$.
\begin{quote}
{\em Exercise 4: 
...
solution to each would simply be the solution to the respective linear
advection equation.  
...
$r^{(2)}$ is non-zero only for the density element, this then means
that only density jumps across the $\lambda = u$ wave (see for
example, Toro~\cite{toro:1997}, Ch.\ 2, 3 or Leveque~\cite{leveque:2002} for a thorough discussion).  Figure~\ref{fig:sod} shows the three waves 
...
To solve the Euler equations using a high-order Godunov method, we
need left and right states at each interface (centered in time), which
are the input to the Riemann solver.  The Riemann solver will then return
the fluxes through the interface.
This is illustrated in Figure~\ref{fig:riemann}.  The fluxes allow us
to update the state in time as:
\begin{equation}
U^{n+1}_i = U^n_i + \frac{\Delta t}{\Delta x} \left ( F_{i-1/2}^{n+1/2} - F_{i+1/2}^{n+1/2} \right )
\end{equation}
Instead of approximating the flux itself on the interface, we find an approximation
to the state on the interface, $U_{i-1/2}^{n+1/2}$ and $U_{i+1/2}^{n+1/2}$ and
use this with the flux function to define the flux through the interface:
\begin{eqnarray}
F_{i-1/2}^{n+1/2} &=& F(U_{i-1/2}^{n+1/2}) \\
F_{i+1/2}^{n+1/2} &=& F(U_{i+1/2}^{n+1/2})
\end{eqnarray}
We use the conserved variables for the final update, but in
constructing the interface states, it is often easier to work with the
primitive variables and then switch back to the conserved variables.
...
no attempt was made to center it in time, it is first-order accuate in time.
...
consider piecewise linear reconstruction---the cell average data
is approximated by a line within each cell.  Figure~\ref{fig:plm}
shows the piecewise linear reconstruction of some data.
...
Just like for the advection equation, we do a Taylor expansion through
$\Delta x/2$ to bring us to the interface, and $\Delta t/2$ to bring
us to the midpoint in time.  Starting with $q_i$, the cell-centered
primitive variable, expanding to the right interface (to create the
left state there) gives:
\begin{eqnarray}
...
\end{eqnarray}
where $\Delta q_i$ is the reconstructed slope of the primitive variable
in that cell (similar to how we compute it for the advection
equation).  We note that the terms truncated in the first line are $O(\Delta x^2)$ and $O(\Delta t^2)$.
...
interface.  First, we recognize that $A = R \Lambda L$ and replacing the `$1$'
in Eq.~\ref{eq:taylorstate} with the identity, $I = LR$, we rewrite this
expression as:
\begin{equation}
q_{i+1/2,L}^{n+1/2} = q_i^n + 
...
R\Lambda L \overline{\Delta q} =
   \left ( \begin{array}{c|c|c}
             r_1^{(1)} & r_1^{(2)} & r_1^{(3)} \\
             r_2^{(1)} & r_2^{(2)} & r_2^{(3)} \\
             r_3^{(1)} & r_3^{(2)} & r_3^{(3)} \end{array} \right )
   \left ( \begin{array}{ccc}
             \lambda^{(1)} &               & \\
                           & \lambda^{(2)} & \\
                           &               & \lambda^{(3)} \end{array} \right )
   \left ( \begin{array}{ccc}
             l_1^{(1)} & l_2^{(1)} & l_3^{(1)} \\
             \hline 
             l_1^{(2)} & l_2^{(2)} & l_3^{(2)} \\
             \hline
             l_1^{(3)} & l_2^{(3)} & l_3^{(3)} \end{array} \right )
   \left ( \begin{array}{c}
            \overline{\Delta \rho} \\
            \overline{\Delta u} \\
...
R\Lambda L \overline{\Delta q} =
   ...
   \left ( \begin{array}{ccc}
             \lambda^{(1)} &               & \\
                           ...
                           &               & \lambda^{(3)} \end{array} \right )
   \left ( \begin{array}{c}
            l^{(1)} \cdot \overline{\Delta q} \\
            l^{(2)} \cdot \overline{\Delta q} \\
            l^{(3)} \cdot \overline{\Delta q} \end{array} \right )
\renewcommand{\arraystretch}{1.0}
\end{equation}
...
R\Lambda L \overline{\Delta q} =
   \left ( \begin{array}{c|c|c}
             ...
             r_3^{(1)} & r_3^{(2)} & r_3^{(3)} \end{array} \right )
   \left ( \begin{array}{c}
            \lambda^{(1)} \, l^{(1)} \cdot \overline{\Delta q} \\
            \lambda^{(2)} \, l^{(2)} \cdot \overline{\Delta q} \\
            \lambda^{(3)} \, l^{(3)} \cdot \overline{\Delta q} \end{array} \right )
\renewcommand{\arraystretch}{1.0}
\end{equation}
Finally, the last multiple results in a column vector with each component
taking the respective component from the each right eigenvector with the vector,
giving:
\begin{equation}
\renewcommand{\arraystretch}{1.5}
R\Lambda L \overline{\Delta q} =
   \left ( \begin{array}{c}
            r_1^{(1)} \, \lambda^{(1)} \, l^{(1)} \cdot \overline{\Delta q} +
            r_1^{(2)} \, \lambda^{(2)} \, l^{(2)} \cdot \overline{\Delta q} +
            r_1^{(3)} \, \lambda^{(3)} \, l^{(3)} \cdot \overline{\Delta q} \\
%
            r_2^{(1)} \, \lambda^{(1)} \, l^{(1)} \cdot \overline{\Delta q} +
            r_2^{(2)} \, \lambda^{(2)} \, l^{(2)} \cdot \overline{\Delta q} +
            r_2^{(3)} \, \lambda^{(3)} \, l^{(3)} \cdot \overline{\Delta q} \\
%
            r_3^{(1)} \, \lambda^{(1)} \, l^{(1)} \cdot \overline{\Delta q} +
            r_3^{(2)} \, \lambda^{(2)} \, l^{(2)} \cdot \overline{\Delta q} +
            r_3^{(3)} \, \lambda^{(3)} \, l^{(3)} \cdot \overline{\Delta q} \\
   \end{array} \right )
\renewcommand{\arraystretch}{1.0}
\end{equation}
We see that this is simply the column vector:
\begin{equation}
...
its wave is moving toward the interface.
...
each wave carries a jump in $r^{(\nu)}$ and we add only those jumps
moving toward the interface to our interface state.  This restriction
of only summing up the waves moving toward the interface is sometimes
called {\em characteristic tracing}.  This decomposition in terms of
the eigenvectors and eigenvalues is commonly called a {\em
...
the interface state and the characteristic decomposition simply adds
up all these jumps, then we can subtract off the reference state and
...
reaches the interface.  Colella \& Glaz (p.\ 278) and Colella (Eq.\ 2.11) suggest
\begin{equation}
q_\mathrm{ref} = \tilde{q}_{i,L} \equiv q_i + \frac{1}{2} \left [ 1 - \frac{\Delta t}{\Delta x}
 \max(\lambda_i^+, 0) \right ] \overline{\Delta q}_i \label{eq:qref}
\end{equation}
where $\lambda^+$ is the largest eigenvalue.  Physically, this
reference state represented the jump carried by the fastest wave
moving toward the interface.  Then,
\begin{equation}
q_{i+1/2,L}^{n+1/2} - \tilde{q}_{i,L} = \frac{1}{2} \frac{\Delta t}{\Delta x}
  \left [ \max(\lambda_i^+,0) - A_i \right ] \overline{\Delta q}_i
\end{equation}
and projecting this RHS (see Colella \& Glaz Eq. 43; Miller \& Colella Eq. 87),
and isolating the interface state, we have
\begin{eqnarray}
q_{i+1/2,L}^{n+1/2} &=& \tilde{q}_{i,L} + \frac{1}{2} \frac{\Delta t}{\Delta x}
       \sum_{\nu; \lambda^{(\nu)} \ge 0} l_i^{(\nu)} \cdot \left [ \max(\lambda_i^+,0) - A_i \right ]
                                           \overline{\Delta q}_i \, r_i^{(\nu)} \\
                    &=& \tilde{q}_{i,L} + \frac{1}{2} \frac{\Delta t}{\Delta x}
       \sum_{\nu; \lambda^{(\nu)} \ge 0} \left [ \max(\lambda_i^+,0) - \lambda_i^{(\nu)} \right ]
                                          (l_i^{(\nu)} \cdot \overline{\Delta q}_i) \, r_i^{(\nu)}
\end{eqnarray}
This is equivalent to the expression in Saltzmann~\cite{saltzman:1994}
...
q_{i+1/2,R}^{n+1/2} = \tilde{q}_{i+1,R} + \frac{1}{2} \frac{\Delta t}{\Delta x}
       \sum_{\nu; \lambda^{(\nu)} \ge 0} 
       \left [ \min(\lambda_{i+1}^-,0) - \lambda_{i+1}^{(\nu)} \right ]
       (l_{i+1}^{(\nu)} \cdot \overline{\Delta q}_{i+1}) \, r_{i+1}^{(\nu)}
\end{equation}
...
$\lambda^-$, the leftmost eigenvalue):
\begin{equation}
\tilde{q}_{i+1,R} = q_{i+1} - \frac{1}{2} \left [ 1 + \frac{\Delta t}{\Delta x} \min(\lambda_{i+1}^-,0) \right ] \overline{\Delta q}_{i+1}
\end{equation}
...
paper~\cite{colellawoodward:1984}, there have been many dicussions of
...
\caption{\label{fig:ppm_trace} Integration under the parabolic
  ...
  $I_+$ (this is indicated by the shaded region).  The $I_+$ carried
...
\right .
\end{equation}
where the superscript $(+)$ on $\mathcal{I}$ indicates that the fastest eigenvalue
($\lambda^+ = u + c$) is used.  This
is similar in spirit to Eq.~\ref{eq:qref}.  Note: in the original PPM
paper, if the wave is not approaching the interface, instead of
using the cell-average, $q_i$, they use the limit of the quadratic
interpolant.  In contrast to the above, the Castro
paper~\cite{almgren:2010} just uses $q_i$ for the reference state
regardless of whether the wave is moving toward or away from the
interface.  Note that if the system were linear, then the choice of
reference state would not matter.
...
{\em Aside}: 
Note that this characterstic projection of $\tilde{q}_+ -
...
Eq.~\ref{eq:ppmtrace}, and writing the reference state as $\tilde{q}$,
...
\begin{eqnarray}
 u_{i+1/2,L}^{n+1/2} =
     \tilde{u} &-& 
       ...
       \right ) 
    \left ( \begin{array}{c} 
           \tilde{\rho} - \mathcal{I}_+^{(1)}(\rho) \\
           \tilde{u} - \mathcal{I}_+^{(1)}(u) \\
           \tilde{p} - \mathcal{I}_+^{(1)}(p) 
            \end{array} \right )
...
%
     &-&\left ( \begin{array}{ccc} 
       ...
       \right ) 
    \left ( \begin{array}{c} 
           \tilde{\rho} - \mathcal{I}_+^{(2)}(\rho) \\
           \tilde{u} - \mathcal{I}_+^{(2)}(u) \\
           \tilde{p} - \mathcal{I}_+^{(2)}(p) 
            \end{array} \right )
    {\color{mygray} \left ( \begin{array}{c} 
           1  \\
           {\color{black} 0} \\
...
%
    &-&\left ( \begin{array}{ccc} 
       ...
       \right ) 
    \left ( \begin{array}{c} 
           \tilde{\rho} - \mathcal{I}_+^{(3)}(\rho) \\
           \tilde{u} - \mathcal{I}_+^{(3)}(u) \\
           \tilde{p} - \mathcal{I}_+^{(3)}(p) 
            \end{array} \right )
    {\color{mygray} \left ( \begin{array}{c} 
           0  \\
           {\color{black} c/\rho} \\
...
%
\end{eqnarray}
Here again we show the entire right eigenvector, but only the element
that comes into play is drawn in black.  This shows that the second
term is $0$---the contact wave does not carry a jump in velocity.  Multiplying
out $l^{(\nu)} \cdot (\tilde{q} - \mathcal{I}_+^{(\nu)})$ we have:
\begin{equation}
u_{i+1/2,L}^{n+1/2} =
   \tilde{u} 
  - \frac{1}{2} \left [
      (\tilde{u} - \mathcal{I}_+^{(1)}(u) ) - 
       \frac{\tilde{p} - \mathcal{I}_+^{(1)}(p)}{C} \right ]
  - \frac{1}{2} \left [
      (\tilde{u} - \mathcal{I}_+^{(3)}(u) ) +
       \frac{\tilde{p} - \mathcal{I}_+^{(3)}(p)}{C} \right ]
\end{equation}
...
$\tilde{C}$).  This may be following the comment before Eq. 3.6,
...
and each wave will carry with it a jump in the fluid variables.  The
...
supersonic wave), or the contact (the middle wave) can be on either
side.
...
pressure and velocity are constant across that wave.  We determine the
state in the star region ($\rho_l^*, \rho_r^*, u^*, p^*$) by using
the jump conditions for the Euler equations.  In general, these differ
depending on whether the waves are shocks or rarefactions.  In practice,
approximate Riemann solvers often assume one or the other (for example,
the two-shock Riemann solver used here~\cite{colellaglaz:1985}).  With
the wave speeds and the states know in each region, we can evaluate the
state on the interface, $q_{i+1/2}^{n+1/2}$.
...
so Riemann solvers (e.g.\ the HLL(C) solvers) instead approximate the
fluxes directly.
...
\item {\em Artifical viscosity}: Colella and Woodward argue discuss
  that behind slow-moving shocks these methods can have oscillations.
  The fix they propose is to use some artifical viscosity---this is
  additional dissipation that kicks in at shocks.  (They argue that
  flattening alone is not enough).  

  We use a multidimensional analog of their artifical viscosity
...
\item {\em Contact steepening}: in contrast to shocks, contact waves
  ...
  paper advocates a contact steepening method to artifically steepen
...
\end{equation}
we predict to the edges in the same fashion as descibed above, but now
...
second-order accurate.  Nevertheless, some sources (e.g.\ the PPM
paper) use edge-centered values for the gravitational source.  Note
also that in the original PPM paper, they include the source terms in
the characteristic projection (see \cite{colellawoodward:1984}, Eq. 3.7,
but note that there the full acceleration over the timestep is used
while most sources have a factor of 1/2).
...
adabatic index (they call $\Gamma$, others call $\gamma_c$) appears in
...
overspecifies the thermodynamics, but eliminates the need for
$\gamma_e$.
