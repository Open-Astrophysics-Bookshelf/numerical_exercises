...
the change in sign of the funciton $f(x)$.  Brendt's method combines
...
\end{equation}
This is more complicated to solve, and generally will require some linear
algebra.  If we take $\Delta t$ to be small, then the change in the solution,
$\Delta {\bf y}$ will be small as well, and we can Taylor-expand the system.
...
\end{equation}
 ...
 and since $\Delta {\bf y} = {\bf y}^{n+1} - {\bf y}^n$,
our method becomes:
\begin{equation}
...
which the solution changes~\cite{byrnehindmarsh}\footnote{Defining whether a problem is stiff can be tricky.  For a system of ODEs, a large range in the eigenvalues of the Jacobian usually means it is stiff}.  A good example
