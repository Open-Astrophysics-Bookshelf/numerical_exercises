...
\begin{equation}
  ...
  q^{(\text{Fifth})}_{i+\myhalf} = \tfrac{1}{60} \left( 2 \langle q \rangle_{i-2} - 13 \langle q \rangle_{i-1} + 47 \langle q \rangle_{i} + 27 \langle q \rangle_{i+1} - 3 \langle q \rangle_{i+2} \right) + {\cal O}(\Delta x^5).
\end{equation}
Alternatively we could use only three of the points, in three different ways:
\begin{subequations}
  \label{eq:weno-advection-3pt}
  \begin{align}
    q^{(\text{Third}, 2)}_{i+\myhalf} &= \tfrac{1}{6} \left( 2 \langle q \rangle_{i-2} - 7 \langle q \rangle_{i-1} + 11 \langle q \rangle_{i} \right) + {\cal O}(\Delta x^3), \\
    q^{(\text{Third}, 1)}_{i+\myhalf} &= \tfrac{1}{6} \left( - \langle q \rangle_{i-1} + 5 \langle q \rangle_{i} + 2 \langle q \rangle_{i+1} \right) + {\cal O}(\Delta x^3), \\
    q^{(\text{Third}, 0)}_{i+\myhalf} &= \tfrac{1}{6} \left( 2 \langle q \rangle_{i} + 5 \langle q \rangle_{i+1} - \langle q \rangle_{i+2} \right) + {\cal O}(\Delta x^3).
  \end{align}
\end{subequations}
...
the scheme. This section uses the notation of Gerolymos et al., and we will drop the integral average notation (so $\langle q \rangle_i$ will be denoted $q_i$).
