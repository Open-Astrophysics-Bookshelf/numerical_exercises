...
\end{equation}
Note that when written like this, $\phi$ has units of $p^\prime/\beta_0$. \MarginPar{there is a $\Delta t$ here too}
...
a constraint based on the buoyancy forcing is used:
\begin{equation}
\Delta t_\mathrm{force} = \min \left \{ \left ( 2 \Delta x \frac{\rho}{\rho^\prime g} \right )^{1/2} \right \}
\end{equation}
This is based simply on $\Delta x = (1/2) g_\mathrm{eff} t^2$---the
